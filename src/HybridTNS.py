...
class RealSpace:
    ...
    def __init__(self, params,
                 Npo,kmin=1e-5, kmax=100.0,kcorr = np.logspace(np.log10(1e-4),np.log10(50),256),Om0=0.31):
    ...
    def get_sigmaR(self,r):
        s8 = np.sqrt(integrate.quad(self.integrand, 1e-5,10,args=(r))[0])
        return s8

    def set_global_params(self,params,z):
        pars = {}
        pars['Om0']=self.cosmo.Om_m(0)
        pars['z'] = z
        pars['Om']=self.cosmo.Om_m(z)
...
class model(RealSpace):
    def __init__(self, params,lmax=4,FoG_type="Lorentzian",Npo=1024,saveout=None,filepath=None):
        ...
        if os.path.exists(self.filename):
            self.read_calc()         
            model = main.TNS(self.theory,self.cosmoparams,lmax,FoG_type)
        ...
        self.model = model
        self.init()
        self.Dsigma= Dsigmat.model(self.cosmoparams)
...
# # Dsigma.py not 
 #   def DeltaSigma(self,*args,r0=0):
 #       self.r,self.xi_gm = self.model.xi_gm(*args)
 #       rp,Dsigma = self.Dsigma.compute_DSigma(self.r,self.xi_gm)
 #       if r0 == 0:
 #           return rp,Dsigma
 #       else :
 #           DSigma_cut = self.Dsigma.DSigma_cut(r0)
 #           return rp,DSigma_cut

    def DeltaSigma(self,*args,r0=0,rp=np.geomspace(1e-1,100,100)):
        rp,Dsigma = self.model.DeltaSigma(*args,r0=r0,rp=rp)
        return rp,Dsigma
